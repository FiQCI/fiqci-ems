...
User-facing changes are expected to land with a `CHANGELOG.md` entry under the top `[WIP]` heading (grouped per PR, each group ending in its `https://github.com/FiQCI/fiqci-ems/pull/N` link) and with the prose docs in `docs/pages/` updated — feature guides are Markdown (`zero_noise_extrapolation.md`, `readout_mitigation.md`, `dynamical_decoupling.md`, `pauli_twirling.md`), interface references are reST (`FiQCIEstimatorUsage.rst`, `FiQCISamplerUsage.rst`).
...
- **`FiQCIBackend`** (`backend/core.py`) — the core engine. Wraps an `IQMBackendBase`, delegates unknown attributes to it via `__getattr__`, and is what the other two interfaces build on. Sampler-style mitigation levels. The `backend/` package splits into `core.py` (the backend), `jobs.py` (`BatchedJob`, `MitigatedJob`, `PartialBatch`, `BatchFailedError`), and `counts.py` (count-key reshaping helpers).
...
- **ZNE (Zero-Noise Extrapolation)** — `mitigators/zne.py` (extrapolation fns) + `transpiler_passes/zne_circuits.py` (local/global gate folding). Estimator-only. Scale factors may be any reals ≥ 1 (non-odd-integer values are only *approximated* by partial folding, so extrapolation uses the **achieved** factors, exposed per pair via the job) and may be given per circuit as a list of lists. `extrapolation_method` accepts the built-in strings or a user callable — see below.
...
- `FiQCIEstimatorJob` — wraps the job and a deferred `_compute` fn; `expectation_values()` / `raw_expectation_values()` / `standard_errors()` compute lazily and cache (one `_compute` call fills all three).
...
Raw (pre-mitigation) counts are available via `FiQCIBackend.raw_counts` but only **after** `result()` has been called (post-processing is lazy). Mitigated results carry `fiqci_ems` metadata in each result header: `raw_counts`, plus M3's `quasi_probabilities` (unprojected, keyed like the counts), `mitigation_overhead` and `clipped_outcomes`. The sampler's counts are the projection onto the nearest physical distribution (they must be non-negative integers); the estimator reads the quasi-probabilities instead, since clipping pins ⟨P⟩ towards ±1. Every job handle also exposes `mitigator_options` — the settings frozen at submission, as opposed to the live, mutable `FiQCIBackend.mitigator_options` / `FiQCIEstimator.mitigator_options`.

### Estimator standard errors

`FiQCIEstimatorJob.standard_errors()` mirrors the shape of `expectation_values()`: one dict per circuit/observable pair with `shot_error` (per-Pauli-term `sqrt((1 - ⟨P⟩²) / N)` over the **raw** counts, inflated by `sqrt(mitigation_overhead)` when REM ran, and taken at the unfolded scale-1 point when ZNE is on), `zne_extrapolation_error`, and `total` (the SE of the value actually returned — not a quadrature sum). Only statistical error is reported; ZNE model bias is not.

Each built-in extrapolator is a linear map from the per-scale values to the zero-noise estimate, so it takes an optional `sigmas=` argument and returns `(values, standard_errors)` when given it. User-supplied `extrapolation_method` callables follow the same convention: `_apply_custom_extrapolation` in `primitives/fiqci_estimator.py` passes `sigmas=` only when the callable's signature accepts it (so two-argument callables keep working) and accepts either bare values or a `(values, errors)` pair. Callables reporting no errors leave `zne_extrapolation_error` and `total` as `None`.

### Count-key handling subtlety

M3 expects spaceless bitstrings with one bit per measured qubit, but Qiskit result keys include all classical bits, register spaces, and zero-filled unmeasured bits. `_key_layout` / `_reduce_counts` / `_expand_counts` in `backend/counts.py` strip these down for M3 correction and restore the original key structure afterward. With twirling, `_trim_result_to_groups` keeps the representative (original-circuit) entry per group at the correct stride so result headers stay aligned.
