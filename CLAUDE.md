...
Raw (pre-mitigation) counts are available via `FiQCIBackend.raw_counts` but only **after** `result()` has been called (post-processing is lazy). Mitigated results carry `fiqci_ems` metadata in each result header. Every job handle also exposes `mitigator_options` — the settings frozen at submission, as opposed to the live, mutable `FiQCIBackend.mitigator_options` / `FiQCIEstimator.mitigator_options`.

### Estimator standard errors

`FiQCIEstimatorJob.standard_errors()` mirrors the shape of `expectation_values()`: one dict per circuit/observable pair with `shot_error` (per-Pauli-term `sqrt((1 - ⟨P⟩²) / N)`, taken at the unfolded scale-1 point when ZNE is on), `zne_extrapolation_error`, and `total` (the SE of the value actually returned — not a quadrature sum). Only statistical error is reported; ZNE model bias and the M3 overhead are not.
