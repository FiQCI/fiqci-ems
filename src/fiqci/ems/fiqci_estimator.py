...
class FiQCIEstimator():
    ...
    def _run(self, circuits: QuantumCircuit | list[QuantumCircuit], observables: SparsePauliOp | list[SparsePauliOp], shots: int=2048, **options):
        ...
        if isinstance(observables, list) and isinstance(circuits, list):
            ...
            # if lengths match, we pair them elementwise
                ...
                obs_circuits = [_get_obs_subcircuits([circ], _combine_pauli_ops(obs), ops) for circ, obs in zip(circuits, observables)]
        ...
        for i, obs_circ_groups in enumerate(obs_circuits):
            ...
            measurement_settings = _combine_pauli_ops(observables if isinstance(observables, SparsePauliOp) else observables[i])
